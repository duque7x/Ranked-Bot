...
export async function image(guildApi: Guild, interaction: StringSelectMenuInteraction, client: Bot, vl: string) {
    let value = vl as "title";

    const baseEmbed = EmbedBuilder.from(interaction.message.embeds[0]);
    const translated = "a **imagem** desejada.";
    ...
    collector2.on('collect', async mess => {
        ...
        baseEmbed.data[value as "image"] = { url: content };
