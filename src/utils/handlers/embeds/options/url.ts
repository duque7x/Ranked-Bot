...
export async function url(guildApi: Guild, interaction: StringSelectMenuInteraction, client: Bot, vl: string) {
    let value = vl as "title";

    const baseEmbed = EmbedBuilder.from(interaction.message.embeds[0]);
    const translated = "a **url** desejada.";
    ...
    collector2.on('collect', async mess => {
        ...
        baseEmbed.data[value as "url"] = content;
